...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    desc.add_options()
        ("help", "help message")
        ("usrp_args", po::value<std::string>(&usrp_args)->default_value("addr0=192.168.10.2,addr1=192.168.10.4,recv_buff_size=500000000"), "two uhd device addresses args")
        ("clock_args",po::value<std::string>(&clock_args)->default_value("addr=192.168.10.3"),"octoclock address args")
        ("outdir", po::value<std::string>(&outdir)->default_value("/mnt/ramdisk/hf25"), "output directory")
    ...
    Digital_rf_write_object * data_object0 = NULL; /* main object created by init */
    Digital_rf_write_object * data_object1 = NULL;
   ...
   /* std::cout << boost::format("Running %d comparisons at random intervals.") % num_tests
    ...
    std::string ch_dir0 = outdir+"/ch0";
    std::string ch_dir1 = outdir+"/ch1";
    //    std::cout << ch_dir << std::endl;
    std::cout << "Writing complex short to multiple files and subdirectores in " << ch_dir0 << ch_dir1 << std::endl;

    std::string mkdir_cmd0 = "mkdir -p "+ch_dir0;
    std::string mkdir_cmd1 = "mkdir -p "+ch_dir1;
    std::cout << mkdir_cmd0 << std::endl;
    std::cout << mkdir_cmd1 << std::endl;
    //    exit(0);
    result = system(mkdir_cmd0.c_str());
    result = system(mkdir_cmd1.c_str());
    //   result = system("rm -Rf /dev/shm/hf25/cha/2*/tmp*.h5");

    /* init */
    data_object0 = digital_rf_create_write_hdf5((char *)ch_dir0.c_str(),
					       H5T_NATIVE_SHORT,
					       subdir_cadence,
    ...
    data_object1 = digital_rf_create_write_hdf5((char *)ch_dir1.c_str(),
                                               H5T_NATIVE_SHORT,
                                               subdir_cadence,
                                               millseconds_per_file,
                                               global_start_index,
    ...
    Digital_rf_write_object data_objects[] = {data_object0,data_object1};
    ...
    while (1)
    {
      ...
      for (size_t ch = 0; ch < rx_stream->get_num_channels(); ch++){
        ...
        if(num_rx_samps  == 363){
          ...
          {
            //    printf("%d\n",data_short[0]);
            result = digital_rf_write_hdf5((data_objects[ch], vector_leading_edge_index + packet>
            packet_i+=1;
          }
          else
          {
            ...
            {
              result = digital_rf_write_hdf5((data_objects[ch], vector_leading_edge_index + pack>
              packet_i+=1;
            }
